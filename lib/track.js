...
//
// Meta: when the pixel is loaded (NEXT_PUBLIC_META_PIXEL_ID), generate_lead is
// sent as the standard Lead event (eventID = lead_id) and form_start as the
// custom FormStart event, for the started-but-did-not-submit audience.
//
...
export function track(event, params = {}) {
  ...
  if (typeof window.fbq === 'function') {
    if (event === 'generate_lead') window.fbq('track', 'Lead', { content_name: params.enquiry || '' }, { eventID: params.lead_id || '' })
    else if (event === 'form_start') window.fbq('trackCustom', 'FormStart', { form_variant: params.form_variant || '' })
  }
