...
const DEFAULT_EXCLUDE = [
  // Internal / private apps — never public
  'bluepeek-dashboard',
  'bluepeek-crm',
  'bluepeek-forms',
  'bluepeek-reviews',
  ...
  'bluepeek-website',
  '_client-template',
  // Retired from the gallery by request. Removing these from CLIENT_SITES is
  // NOT enough — the Vercel sync re-adds any project it can still see, so a
  // withdrawn client has to be named here too.
  'ae-ac-service-center',
  'aeandac',
  'aeandac-service-center',
  'barberz-den',
  'barberzden',
  'bluestar-mechanics',
  'bluestar',
  // Demos and templates that are not real client work
  'gym-growth-demo',
  'tvr-demo',
  'medicube-landing',
  'browser-use-demo',
...
// Domains withdrawn from the gallery. Project-name exclusion is not enough:
// the same site can sit under a different project name (or another Vercel
// team), so match what actually renders — the domain.
const DEFAULT_EXCLUDE_DOMAINS = [
  'aeandacservicecenter.com.au',
  'ae-ac-service-center.vercel.app',
  'aeandac.vercel.app',
  'barberzden.com.au',
  'barberz-den.vercel.app',
  'bluestarmechanics.com.au',
]

function excludeDomains() {
  const extra = (process.env.GALLERY_EXCLUDE_DOMAINS || '')
    .split(',').map(s => normDomain(s.trim())).filter(Boolean)
  return new Set([...DEFAULT_EXCLUDE_DOMAINS.map(normDomain), ...extra])
}
...
function normDomain(url = '') {
  const domain = url
    ...
    .replace(/\/+$/, '')
    .toLowerCase()
  // The same recent build is supplied under both its preview and live domains.
  return domain === 'coastal-2pac.vercel.app' ? 'coastal2pac.com.au' : domain
}
...
async function fetchVercelSites(token) {
  ...
  const excludedDomains = excludeDomains()
  ...
  return projects
    ...
    .map(p => {
      ...
      if (!url) return null
      if (excludedDomains.has(normDomain(url))) return null
      return {
...
function mergeGallery(curated, vercelSites) {
  const dropped = excludeDomains()
  const keep = s => !dropped.has(normDomain(s.url))
  const curatedKept = curated.filter(keep)
  const have = new Set(curatedKept.map(c => normDomain(c.url)))
  const extras = vercelSites.filter(v => keep(v) && !have.has(normDomain(v.url)))
  return [...curatedKept, ...extras]
}
...
export async function getGallerySites() {
  ...
  let sites = mergeGallery(CLIENT_SITES, [])
