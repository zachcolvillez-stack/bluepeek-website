          ...
          <div className="space-y-6">
            {BLOG_POSTS.map((p) => (
              <Link key={p.slug} href={`/blog/${p.slug}`}
                ...
                <p className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#7e8aa0' }}>
                  {new Date(p.date).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })} · {p.readTime}
                </p>
                <h2 className="text-xl md:text-2xl font-bold tracking-tight mb-3" style={{ color: '#0c1c34' }}>{p.title}</h2>
                <p className="text-base leading-relaxed mb-4" style={{ color: '#475569' }}>{p.description}</p>
