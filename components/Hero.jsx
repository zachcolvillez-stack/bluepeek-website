  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center pt-28 pb-20 px-6 overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <img
          src="/office-bg.png"
          alt="Premium high-rise office at night overlooking a city skyline"
          className="w-full h-full object-cover"
        />
        {/* Scrim — darker on the left for text legibility, fades into the page at the bottom */}
        <div className="absolute inset-0" style={{
          background:
            'linear-gradient(90deg, rgba(8,11,20,0.94) 0%, rgba(8,11,20,0.80) 34%, rgba(8,11,20,0.42) 66%, rgba(8,11,20,0.55) 100%),' +
            'linear-gradient(180deg, rgba(8,11,20,0.55) 0%, transparent 22%, transparent 55%, #080b14 100%)',
        }} />
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto w-full">
        <div className="max-w-2xl">
          ...
          <motion.h1 initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.08 }}
            className="text-4xl md:text-5xl lg:text-[3.6rem] font-bold tracking-tight leading-[1.07] mt-6 mb-6"
            style={{ color: '#f4f7fd' }}>
          ...
          <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.16 }}
            className="text-base md:text-lg leading-relaxed mb-8 max-w-xl" style={{ color: '#c3ccdd' }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.4 }}
            ...
            {TRUST.map(t => (
              <div key={t} className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium" style={{ color: '#c3ccdd' }}>{t}</span>
              </div>
            ))}
          </motion.div>
        </div>
