...
export default function Contact() {
  ...
  const inpStyle = { background: 'var(--bg-2)', border: '1px solid rgba(6,20,63,0.12)', color: 'var(--ink)' }
  const onFocus = (e) => { e.target.style.borderColor = 'var(--ink)' }
  const onBlur  = (e) => { e.target.style.borderColor = 'rgba(6,20,63,0.12)' }
    ...
    <section id="contact" className="section-tint relative py-28 px-6">
      <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-start">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          <p className="text-base md:text-lg leading-relaxed mb-8" style={{ color: 'var(--text)' }}>
            ...
            {TRUST.map(t => (
              <div key={t.label} className="flex items-center gap-3">
                <span className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                  style={{ background: 'rgba(11,62,217,0.09)' }}>
                  <t.icon size={16} style={{ color: 'var(--lapiz-deep)' }} />
                </span>
          ...
          <a href={`mailto:${CONTACT_EMAIL}`}
            ...
            style={{ background: 'var(--bg-2)', border: '1px solid rgba(6,20,63,0.12)', color: 'var(--ink)' }}>
            <Mail size={16} style={{ color: 'var(--lapiz-deep)' }} />
            {CONTACT_EMAIL}
          </a>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          {status === 'sent' ? (
            <div className="text-center py-12">
              <div className="w-14 h-14 rounded-2xl mx-auto mb-4 flex items-center justify-center" style={{ background: 'rgba(11,62,217,0.09)' }}>
                <CheckCircle size={28} style={{ color: 'var(--lapiz-deep)' }} />
              </div>
              <h3 className="text-xl font-bold mb-2" style={{ color: 'var(--ink)' }}>Thanks - we’ve got it.</h3>
              <p className="mb-6" style={{ color: 'var(--text)' }}>We’ll be in touch within one business day.</p>
              <button onClick={() => setStatus('idle')} className="text-sm font-semibold" style={{ color: 'var(--lapiz-deep)' }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs mb-1.5 font-semibold" style={{ color: 'var(--text)' }}>Your Name *</label>
                  <input required className={inp} style={inpStyle} placeholder="John Smith" onFocus={onFocus} onBlur={onBlur}
                    value={form.name} onChange={e => set('name', e.target.value)} />
                </div>
                <div>
                  <label className="block text-xs mb-1.5 font-semibold" style={{ color: 'var(--text)' }}>Business Name *</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs mb-1.5 font-semibold" style={{ color: 'var(--text)' }}>Email *</label>
                ...
                <div>
                  <label className="block text-xs mb-1.5 font-semibold" style={{ color: 'var(--text)' }}>Phone</label>
                  <input type="tel" className={inp} style={inpStyle} placeholder="04xx xxx xxx" onFocus={onFocus} onBlur={onBlur}
                    value={form.phone} onChange={e => set('phone', e.target.value)} />
                </div>
              </div>
              <div>
                <label className="block text-xs mb-1.5 font-semibold" style={{ color: 'var(--text)' }}>Tell us about your business</label>
              ...
              <p className="text-xs text-center pt-1" style={{ color: 'var(--muted)' }}>
                Or email us at{' '}
                <a href={`mailto:${CONTACT_EMAIL}`} className="font-semibold" style={{ color: 'var(--lapiz-deep)' }}>
                  {CONTACT_EMAIL}
                </a>
              </p>
            </form>
