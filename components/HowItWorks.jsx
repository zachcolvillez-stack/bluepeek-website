...
const STEPS = [
  { number: '01', icon: Search,    title: 'Discovery',        description: 'A relaxed conversation about your business, your goals and what success looks like. If Bluepeek isn’t the right fit, we’ll tell you upfront.' },
    ...
    <section id="how-it-works" className="section-tint relative py-28 px-6">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          <p className="text-base md:text-lg max-w-lg mx-auto leading-relaxed" style={{ color: 'var(--text)' }}>
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-9 left-[12.5%] right-[12.5%] h-0.5"
            style={{ background: 'linear-gradient(90deg, transparent, rgba(6,20,63,0.18) 15%, rgba(6,20,63,0.18) 85%, transparent)' }} />
              ...
              >
                <div className="relative inline-flex items-center justify-center mb-5">
                  <div className="rounded-2xl flex items-center justify-center"
                    style={{ width: '72px', height: '72px', background: 'linear-gradient(135deg, var(--lapiz-deep) 0%, var(--ink) 100%)', boxShadow: '0 12px 28px rgba(6,20,63,0.28)' }}>
                    <step.icon size={26} className="text-white" />
                  </div>
                  <span className="absolute -top-2 -right-1 w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold"
                    style={{ background: '#ffffff', color: 'var(--lapiz-deep)', border: '1.5px solid rgba(6,20,63,0.14)' }}>
                    {step.number}
                  </span>
                </div>

                <h3 className="text-lg font-bold mb-2.5" style={{ color: 'var(--ink)' }}>{step.title}</h3>
