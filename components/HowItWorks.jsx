      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-9 left-[12.5%] right-[12.5%] h-0.5"
            style={{ background: 'linear-gradient(90deg, transparent, rgba(12,28,52,0.18) 15%, rgba(12,28,52,0.18) 85%, transparent)' }} />
              ...
              >
                <div className="relative inline-flex items-center justify-center mb-5">
                  <div className="rounded-2xl flex items-center justify-center"
                    style={{ width: '72px', height: '72px', background: 'linear-gradient(135deg, #16335c 0%, #0c1c34 100%)', boxShadow: '0 12px 28px rgba(12,28,52,0.28)' }}>
                    <step.icon size={26} className="text-white" />
                  </div>
                  <span className="absolute -top-2 -right-1 w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold"
                    style={{ background: '#ffffff', color: '#16335c', border: '1.5px solid rgba(12,28,52,0.14)' }}>
                    {step.number}
                  </span>
                </div>

                <h3 className="text-lg font-bold mb-2.5" style={{ color: '#0c1c34' }}>{step.title}</h3>
                <p className="text-sm leading-relaxed" style={{ color: '#475569' }}>{step.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
