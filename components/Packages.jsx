    ...
    <section id="packages" className="relative py-28 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          {PACKAGES.map((p, i) => (
            <motion.div key={p.name}
              ...
              style={p.popular
                ? { background: 'rgba(127,176,247,0.08)', border: '1.5px solid rgba(159,182,255,0.5)', boxShadow: '0 24px 60px rgba(0,0,0,0.5), 0 0 40px rgba(127,176,247,0.12)', transform: 'scale(1.03)' }
                : { background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.09)', boxShadow: '0 2px 12px rgba(0,0,0,0.35)' }}
            >
              {p.popular && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-full text-white"
                  style={{ background: 'linear-gradient(135deg, #7fb0f7 0%, #8f86f3 55%, #a78bf5 100%)', boxShadow: '0 6px 16px rgba(120,140,255,0.4)' }}>
                  Most Popular
                </span>
              )}

              <h3 className="text-xl font-bold mb-1.5" style={{ color: '#f4f7fd' }}>{p.name}</h3>
              <p className="text-sm mb-6" style={{ color: '#7e889f' }}>{p.tagline}</p>

              {p.hasAI && (
                <div className="flex items-center gap-2 mb-5 px-3 py-2 rounded-xl"
                  style={{ background: 'rgba(159,182,255,0.12)', border: '1px solid rgba(159,182,255,0.24)' }}>
                  <Sparkles size={15} style={{ color: '#9fb6ff' }} />
                  <span className="text-xs font-semibold" style={{ color: '#9fb6ff' }}>Includes 24/7 AI chatbot</span>
                </div>
              )}

              <ul className="space-y-3 mb-8">
                {p.features.map(f => (
                  <li key={f} className="flex items-start gap-2.5 text-sm" style={{ color: '#aab3c9' }}>
        ...
        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}
          ...
          className="text-center text-sm mt-10" style={{ color: '#7e889f' }}>
          Need something different? <a href="#contact" onClick={(e) => { e.preventDefault(); onCTA?.('contact') }} className="font-semibold" style={{ color: '#9fb6ff' }}>Ask for a custom quote →</a>
        </motion.p>
      </div>
    </section>
  )
