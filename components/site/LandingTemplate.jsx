    ...
    <>
      ...
      <main className="relative pt-28">
        ...
        <section className="relative max-w-4xl mx-auto px-6 pb-12">
          ...
          <p className="text-base md:text-lg leading-relaxed max-w-2xl mb-8" style={{ color: '#46557d' }}>{lead}</p>
          ...
          {sections.map((s, i) => (
            <section key={i}>
              ...
              {s.paragraphs?.map((p, j) => (
                <p key={j} className="text-base leading-relaxed mb-4" style={{ color: '#46557d' }}>{p}</p>
              ))}
              {s.bullets?.length > 0 && (
                <ul className="space-y-2.5 mt-5">
                  {s.bullets.map((b, k) => (
                    <li key={k} className="flex items-start gap-3 text-base" style={{ color: '#46557d' }}>
              ...
              {faqs.map((f, i) => (
                <details key={i} className="card p-5 group" style={{ borderRadius: '16px' }}>
                  <summary className="flex items-center justify-between cursor-pointer list-none font-semibold" style={{ color: '#1a1730' }}>
                    {f.q}
                    <ChevronRight size={18} className="transition-transform group-open:rotate-90 flex-shrink-0 ml-3" style={{ color: '#7c89a8' }} />
                  </summary>
                  <p className="text-sm leading-relaxed mt-3" style={{ color: '#46557d' }}>{f.a}</p>
                </details>
              ))}
            </div>
          </section>
