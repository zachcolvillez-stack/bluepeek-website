    ...
    <>
      ...
      <main className="relative pt-28">
        ...
        <section className="relative max-w-4xl mx-auto px-6 pb-10">
          ...
          <p className="text-base md:text-lg" style={{ color: '#475569' }}>{study.location}</p>
        </section>
        ...
        <section className="relative max-w-4xl mx-auto px-6 mb-12">
          <a href={study.url} target="_blank" rel="noopener noreferrer" className="card group block overflow-hidden">
            <div className="flex items-center gap-1.5 px-4 py-2.5" style={{ background: '#eff4fb', borderBottom: '1px solid rgba(12,28,52,0.10)' }}>
              ...
              <div className="flex-1 mx-3 px-3 py-0.5 text-[10px] rounded text-center font-mono truncate"
                style={{ background: '#ffffff', color: '#7e8aa0', border: '1px solid rgba(12,28,52,0.10)' }}>
                {study.url.replace('https://', '').replace('www.', '')}
              </div>
              <ExternalLink size={12} style={{ color: '#7e8aa0' }} />
            </div>
            <div className="relative aspect-[16/9] overflow-hidden" style={{ background: '#eff4fb' }}>
        ...
        <div className="relative max-w-4xl mx-auto px-6 space-y-12">
          <section>
            ...
            <p className="text-base leading-relaxed" style={{ color: '#475569' }}>{content.brief}</p>
          </section>
          <section>
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight mb-4" style={{ color: '#1a1730' }}>What we built</h2>
            <p className="text-base leading-relaxed mb-5" style={{ color: '#475569' }}>{content.built}</p>
            <ul className="space-y-2.5">
              {content.features.map((f, i) => (
                <li key={i} className="flex items-start gap-3 text-base" style={{ color: '#475569' }}>
            ...
            <div className="flex flex-wrap gap-2.5">
              {related.map(r => (
                <Link key={r.href} href={r.href}
                  ...
                  style={{ background: '#ffffff', border: '1px solid rgba(12,28,52,0.10)', color: '#475569' }}>
