    ...
    <>
      ...
      <main className="relative pt-28">
        ...
        <div className="relative max-w-4xl mx-auto px-6">
          <nav aria-label="Breadcrumb" className="flex items-center flex-wrap gap-1.5 text-xs mb-6" style={{ color: '#7c89a8' }}>
            {breadcrumb.map((b, i) => (
              <span key={b.href} className="flex items-center gap-1.5">
                {i > 0 && <ChevronRight size={12} style={{ color: '#7c89a8' }} />}
                {i < breadcrumb.length - 1
                  ? <Link href={b.href} className="transition-colors" style={{ color: '#7c89a8' }} onMouseEnter={(e) => e.currentTarget.style.color = '#1667FC'} onMouseLeave={(e) => e.currentTarget.style.color = '#7c89a8'}>{b.name}</Link>
                  : <span style={{ color: '#46557d' }}>{b.name}</span>}
              </span>
            ))}
          </nav>
        </div>
        ...
        <section className="relative max-w-4xl mx-auto px-6 pb-10">
          ...
          <p className="text-base md:text-lg" style={{ color: '#46557d' }}>{study.location}</p>
        </section>
        ...
        <section className="relative max-w-4xl mx-auto px-6 mb-12">
          <a href={study.url} target="_blank" rel="noopener noreferrer" className="card group block overflow-hidden">
            ...
            <div className="relative aspect-[16/9] overflow-hidden" style={{ background: '#f4f7ff' }}>
        ...
        <div className="relative max-w-4xl mx-auto px-6 space-y-12">
          <section>
            ...
            <p className="text-base leading-relaxed" style={{ color: '#46557d' }}>{content.brief}</p>
          </section>
          <section>
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight mb-4" style={{ color: '#1a1730' }}>What we built</h2>
            <p className="text-base leading-relaxed mb-5" style={{ color: '#46557d' }}>{content.built}</p>
            <ul className="space-y-2.5">
              {content.features.map((f, i) => (
                <li key={i} className="flex items-start gap-3 text-base" style={{ color: '#46557d' }}>
        ...
        {related.length > 0 && (
          <section className="relative max-w-4xl mx-auto px-6 py-12">
            <h2 className="text-sm font-bold uppercase tracking-wider mb-5" style={{ color: '#7c89a8' }}>Explore more</h2>
            <div className="flex flex-wrap gap-2.5">
              {related.map(r => (
                <Link key={r.href} href={r.href}
                  className="inline-flex items-center gap-1.5 text-sm font-medium px-4 py-2 rounded-full"
                  style={{ background: '#ffffff', border: '1px solid rgba(11,62,217,0.14)', color: '#46557d' }}>
